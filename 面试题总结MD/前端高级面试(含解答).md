...
很多情况下数据的变化都不会触发页面的改变，比如：
- data 中未声明的属性不响应，此时用 $set 方法赋值可以触发响应
- 修改数组对象的某一属性不响应，但是在 vue 源码中重写了 push(), pop(), shift(), unshift(), splice(), sort(),reverse() 方法，如果使用以上方法操作数组就会触发响应。或者更直接的方式直接重新赋值数组改变数组的引用地址也会触发响应。
- vue 官方提供了 this.$forceUpdate()方法 （强制刷新）

2. Vue 组件之间传值有哪些方法，父子，非父子？

- 常规父子之间传递值的模型：父传子，props属性，子传父，$emit()
- 兄弟组件直接传值：Vuex (共享数据，任何组件都可以获取其中数据)。使用浏览器 storage 共享数据。eventbus（事件总线，设置 new Vue() 为事件总线，在需要使用的组件引用，就可以用 emit 和 on 发送和接手事件）

3. Vue中的自定义指令如何使用？

自定义指令可以分为全局和局部，全局指令可以直接在 Vue 实例上挂载 Vue.directive('指令名称')，局部指令可以在组件内部直接使用 directives: {} 方法。然后在组件中直接使用新的指令 v-指令名称。

自定义指令有以下钩子函数：bind, inserted, update, componentUpdated, unbind

4. vue.$nextTick用来处理哪类问题？

vue.$nextTick 是在下次 DOM 更新循环结束之后执行延迟回调。通常在修改数据之后立即使用这个方法，获取更新后的 DOM。
